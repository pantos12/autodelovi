...
export default function HomeSearch() {
  ...
  return (
    <>
      <form onSubmit={handleTextSearch} style={{ marginBottom: '16px' }}>
        <label className="sr-only" htmlFor="home-search">Pretrazi delove</label>
      ...
      <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', padding: '20px' }}>
        ...
        <div className="search-bar" style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <select aria-label="Godiste" value={year} onChange={e => setYear(e.target.value)} style={{ ...sel, color: year ? '#fff' : '#888' }}>
          ...
          <button onClick={handleVehicleSearch} aria-label="Pretrazi po vozilu" style={{ background: '#f9372c', color: '#fff', border: 'none', padding: '0 32px', height: '48px', borderRadius: '8px', fontSize: '13px', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer', flexShrink: 0 }}
