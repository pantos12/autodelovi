...
function ComparisonContent() {
  ...
  useEffect(() => {
    ...
    setSearching(true);
    fetch(`/api/search?q=${encodeURIComponent(debouncedSearch)}&per_page=10`, { signal: controller.signal })
      .then(r => r.json())
      ...
      <div style={s.page}>
        <div style={{ ...s.container, textAlign: 'center', paddingTop: '80px' }}>
          ...
          <div style={{ maxWidth: '400px', margin: '0 auto', position: 'relative' }}>
            <input style={{ ...s.input }} placeholder="Pretraži delove..." value={search} onChange={e => setSearch(e.target.value)} />
            {searching && <p style={{ color: '#888', fontSize: '12px', marginTop: '8px' }}>Pretražujem...</p>}
            {searchResults.length > 0 && (
              <div style={{ maxHeight: '300px', overflowY: 'auto', marginTop: '8px' }}>
                {searchResults.map(p => (
                  <div key={p.id} onClick={() => addPart(p.id)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 16px', background: '#1a1b1f', borderRadius: '8px', marginBottom: '8px', cursor: 'pointer', border: '1px solid #252629', transition: 'border-color 0.15s' }}
                    onMouseEnter={e => (e.currentTarget.style.borderColor = '#ff4d00')}
                    onMouseLeave={e => (e.currentTarget.style.borderColor = '#252629')}>
                    <div>
                      <span style={{ color: '#fff', fontSize: '14px', display: 'block' }}>{p.name_sr || p.name}</span>
                      {p.brand && <span style={{ color: '#888', fontSize: '12px' }}>{p.brand}</span>}
                    </div>
                    <span style={{ color: '#ff4d00', fontSize: '14px', fontWeight: 600, flexShrink: 0, marginLeft: '12px' }}>{p.price?.toLocaleString('sr-RS')} RSD</span>
                  </div>
                ))}
              </div>
            )}
            {debouncedSearch.length >= 2 && !searching && searchResults.length === 0 && (
              <p style={{ color: '#666', fontSize: '13px', marginTop: '8px' }}>Nema rezultata za &quot;{debouncedSearch}&quot;</p>
            )}
          </div>
      ...
      <div style={s.container}>
              ...
              <div key={i} style={{ background: '#1a1b1f', borderRadius: '12px', padding: '16px', border: '1px solid #252629', textAlign: 'center', minHeight: '120px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                {part ? (
                  <>
                    ...
                    {part.brand && <p style={{ color: '#888', fontSize: '12px', marginBottom: '4px' }}>{part.brand}</p>}
                  ...
                  <div style={{ width: '100%' }}>
                    ...
                    {searching && <p style={{ color: '#888', fontSize: '11px', marginTop: '4px' }}>...</p>}
                    {searchResults.length > 0 && searchResults.slice(0, 5).map(p => (
                      <div key={p.id} onClick={() => addPart(p.id)} style={{ padding: '6px 10px', background: '#252629', borderRadius: '6px', marginTop: '4px', cursor: 'pointer', width: '100%', textAlign: 'left', transition: 'background 0.15s' }}
                        onMouseEnter={e => (e.currentTarget.style.background = '#333')}
                        onMouseLeave={e => (e.currentTarget.style.background = '#252629')}>
        ...
        {parts.length > 0 && (
          <div style={{ display: 'flex', gap: '12px', marginTop: '24px', flexWrap: 'wrap' }}>
            {parts.map(p => (
              <Link key={p.id} href={`/parts/${p.slug || p.id}`} style={{ padding: '10px 20px', background: '#ff4d00', borderRadius: '8px', color: '#fff', textDecoration: 'none', fontSize: '13px', fontWeight: 600 }}>
                Vidi: {(p.name_sr || p.name).slice(0, 20)}
              </Link>
            ))}
          </div>
        )}
