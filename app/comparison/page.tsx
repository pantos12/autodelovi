...
function ComparisonContent() {
  ...
  const searchParts = useCallback((q: string) => {
    if (q.length < 2) { setSearchResults([]); return; }
    setSearching(true);
    fetch(`/api/search?q=${encodeURIComponent(q)}&per_page=10`)
      .then(r => r.json())
  ...
  function handleSearchInput(q: string) {
    ...
    debounceRef.current = setTimeout(() => searchParts(q), 300);
  }
    ...
    return (
      <div style={s.page}>
        <div style={{ ...s.container, textAlign: 'center', paddingTop: '80px' }}>
          ...
          <input style={{ ...s.input, maxWidth: '400px', margin: '0 auto 16px' }} placeholder="Pretraži delove..." value={search} onChange={e => handleSearchInput(e.target.value)} />
  ...
  return (
    <div style={s.page}>
      <div style={s.container}>
        ...
        `}</style>
        <div className="compare-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '32px' }}>
              ...
              <div key={i} style={{ background: '#1a1b1f', borderRadius: '12px', padding: '16px', border: '1px solid #252629', textAlign: 'center', minHeight: '120px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                {part ? (
                  ...
                  <>
                    ...
                    <input style={{ ...s.input, fontSize: '12px', padding: '6px 10px' }} placeholder="Pretraži..." value={search} onChange={e => handleSearchInput(e.target.value)} />
        ...
        {/* Comparison table */}
        {parts.length > 0 && (
          <div className="compare-table-wrap" style={{ background: '#1a1b1f', borderRadius: '12px', overflow: 'hidden', border: '1px solid #252629' }}>
