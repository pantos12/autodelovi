    ...
    return (
      <div style={s.page}>
        <div style={{ ...s.container, textAlign: 'center', paddingTop: '80px' }}>
          ...
          <h1 style={{ color: '#fff', fontSize: '28px', fontWeight: 800, marginBottom: '12px' }}>Poredenje delova</h1>
          <p style={{ color: '#aaa', marginBottom: '32px' }}>Izaberite do 3 dela za poredenje</p>
          <div style={{ position: 'relative', maxWidth: '400px', margin: '0 auto 16px' }}>
            <input
              style={s.input}
            ...
            />
            {search.length >= 2 && (
              <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: '#1a1b1f', borderRadius: '0 0 8px 8px', border: '1px solid #333', borderTop: 'none', zIndex: 10, maxHeight: '240px', overflowY: 'auto' }}>
                {searching && <div style={{ padding: '12px', color: '#888', fontSize: '13px' }}>Pretrazujem...</div>}
                {!searching && filtered.length === 0 && <div style={{ padding: '12px', color: '#888', fontSize: '13px' }}>Nema rezultata</div>}
                {filtered.map(p => (
                  <div key={p.id} onClick={() => addPart(p.id)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 16px', cursor: 'pointer', borderBottom: '1px solid #252629' }}>
      ...
      <div style={s.container}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '32px' }}>
          {Array.from({ length: 3 }).map((_, i) => {
            const part = parts[i];
            return (
              <div key={i} style={{ background: '#1a1b1f', borderRadius: '12px', padding: '16px', border: '1px solid #252629', textAlign: 'center', minHeight: '120px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                {part ? (
                  <>
                  ...
                  <div style={{ width: '100%' }}>
                    ...
                    <div style={{ position: 'relative' }}>
                      <input style={{ ...s.input, fontSize: '12px', padding: '6px 10px' }} placeholder="Pretrazi..." value={search} onChange={e => setSearch(e.target.value)} />
                      {search.length >= 2 && (
                        <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: '#252629', borderRadius: '0 0 8px 8px', zIndex: 10, maxHeight: '180px', overflowY: 'auto' }}>
                          {searching && <div style={{ padding: '8px', color: '#888', fontSize: '11px' }}>Pretrazujem...</div>}
                          {!searching && filtered.length === 0 && <div style={{ padding: '8px', color: '#888', fontSize: '11px' }}>Nema rezultata</div>}
                          {filtered.slice(0, 5).map(p => (
                            <div key={p.id} onClick={() => addPart(p.id)} style={{ padding: '6px 10px', cursor: 'pointer', borderBottom: '1px solid #333', textAlign: 'left' }}>
