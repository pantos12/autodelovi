    ...
    <div style={{ background: '#0c0d0f', minHeight: '100vh' }}>
      <style>{`
        @media (max-width: 900px) {
          ...
          .pd-buy-card { position: static !important; }
          .pd-image { height: 240px !important; }
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '24px 16px' }}>
        ...
        <div className="pd-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: '32px', alignItems: 'start' }}>
          {/* Left column */}
          <div>
            {/* Image */}
            <div className="pd-image" style={{ position: 'relative', background: '#1a1b1f', borderRadius: '16px', height: '320px', marginBottom: '24px', border: '1px solid #252629', overflow: 'hidden' }}>
              <Image
            ...
            {/* Title */}
            <h1 className="pd-title" style={{ color: '#fff', fontSize: '28px', fontWeight: 800, marginBottom: '8px' }}>{part.name_sr || part.name}</h1>
          ...
          {/* Right: Buy card */}
          <div className="pd-buy-card" style={{ position: 'sticky', top: '80px' }}>
            <div style={{ background: '#1a1b1f', borderRadius: '16px', padding: '24px', border: '1px solid #252629' }}>
              ...
              <Link
                ...
                style={{ display: 'block', width: '100%', padding: '12px', background: '#252629', borderRadius: '10px', color: '#fff', fontSize: '14px', fontWeight: 600, textAlign: 'center', textDecoration: 'none', boxSizing: 'border-box' as const, marginTop: '10px' }}
              >
                ⚖ Uporedi sa drugim delovima
              </Link>
        ...
        {related.length > 0 && (
          <div style={{ marginTop: '48px' }}>
            ...
            <div className="pd-related-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '16px' }}>
              {related.map((rp) => {
                const rpInStock = (rp.stock_quantity ?? 0) > 0;
                return (
                  <Link key={rp.id} href={`/parts/${rp.slug || rp.id}`} style={{ textDecoration: 'none' }}>
                    <div style={{ background: '#1a1b1f', borderRadius: '12px', overflow: 'hidden', border: '1px solid #252629', transition: 'transform 0.15s ease, box-shadow 0.15s ease' }}>
                      ...
                      <div style={{ padding: '12px' }}>
                        ...
                        <p style={{ color: rpInStock ? '#22c55e' : '#ef4444', fontSize: '11px' }}>{rpInStock ? 'Na stanju' : 'Nema na stanju'}</p>
                      </div>
                    </div>
                  </Link>
                );
