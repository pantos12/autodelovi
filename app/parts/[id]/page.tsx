...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      ...(part.images?.[0] ? { images: [{ url: part.images[0] }] } : {}),
    },
  };
}
    ...
    <div style={{ background: '#0c0d0f', minHeight: '100vh' }}>
      ...
      <style>{`
        @media (max-width: 768px) {
          .part-detail-layout { grid-template-columns: 1fr !important; }
          .part-detail-buy-card { position: static !important; }
        }
      `}</style>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '24px 16px' }}>
        {/* Breadcrumb */}
        <nav aria-label="Navigacija" style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '24px', fontSize: '14px', flexWrap: 'wrap' }}>
        ...
        <div className="part-detail-layout" style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: '32px', alignItems: 'start' }}>
          ...
          {/* Right: Buy card */}
          <div className="part-detail-buy-card" style={{ position: 'sticky', top: '80px' }}>
            <div style={{ background: '#1a1b1f', borderRadius: '16px', padding: '24px', border: '1px solid #252629' }}>
              ...
              {part.price_eur && (
                <div style={{ color: '#aaa', fontSize: '14px', marginBottom: '8px' }}>&#8776; &euro;{part.price_eur.toFixed(2)}</div>
              )}
              ...
              <Link
                ...
                style={{ display: 'block', width: '100%', padding: '12px', background: '#252629', borderRadius: '10px', color: '#fff', fontSize: '14px', fontWeight: 600, textAlign: 'center', textDecoration: 'none', boxSizing: 'border-box' as const, marginTop: '12px' }}
              >
                &#8776; Uporedi
              </Link>
              {part.supplier && (
