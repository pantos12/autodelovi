...
export default async function PartDetail({ params }: { params: { id: string } }) {
  ...
  const isPlaceholder = !part.images?.[0];
          ...
          <div className="part-buy-card" style={{ position: 'sticky', top: '80px' }}>
            <div style={{ background: '#1a1b1f', borderRadius: '16px', padding: '24px', border: '1px solid #252629' }}>
              ...
              <Link
                ...
                aria-label="Uporedi sa drugim delovima"
              ...
              {part.source_url && (
                <a href={part.source_url} target="_blank" rel="noopener noreferrer" style={{ display: 'block', textAlign: 'center', marginTop: '12px', color: '#aaa', fontSize: '12px', textDecoration: 'none' }}>
                  Pogledaj na sajtu dobavljaca &rarr;
                </a>
              )}
            </div>
          </div>
        ...
        {related.length > 0 && (
          <section style={{ marginTop: '48px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '16px' }}>
              {related.map((rp) => {
                const rpIsPlaceholder = !rp.images?.[0];
                return (
                  <article key={rp.id} style={{ background: '#1a1b1f', borderRadius: '12px', overflow: 'hidden', border: '1px solid #252629' }}>
                    <div style={{ position: 'relative', background: '#252629', height: '120px', overflow: 'hidden' }}>
                      <Image
                        src={rp.images?.[0] || '/images/part-placeholder.svg'}
                        alt={rp.name_sr || rp.name}
                        fill
                        sizes="(max-width: 768px) 50vw, 220px"
                        style={{ objectFit: 'cover' }}
                        loading="lazy"
                        unoptimized={rpIsPlaceholder}
                      />
                    </div>
