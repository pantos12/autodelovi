...
export async function POST(request: NextRequest) {
  try {
    ...
    const partsMap = new Map(
      (partsData ?? []).map((row) => {
        const p = row as unknown as PartRow & { supplier: { name: string | null } | { name: string | null }[] | null };
        const supplier = Array.isArray(p.supplier) ? (p.supplier[0] ?? null) : p.supplier;
        return [p.id, { ...p, supplier } as PartRow];
      })
    );
