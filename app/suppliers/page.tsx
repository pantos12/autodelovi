...
export default function SuppliersPage() {
  const stats = [
    { value: '200+', label: 'Verifikovanih dobavljača' },
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px' }}>
          {suppliers.map(supplier => (
            <div key={supplier.id} style={{ background: '#1a1b1f', borderRadius: '12px', padding: '24px', border: '1px solid #252629' }}>
              ...
              <p style={{ color: '#888', fontSize: '14px', lineHeight: 1.5, marginBottom: '16px' }}>{supplier.description}</p>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', gap: '16px' }}>
                  <div>
                    <div style={{ color: '#fff', fontSize: '18px', fontWeight: 700 }}>{supplier.parts}</div>
                    <div style={{ color: '#aaa', fontSize: '12px' }}>Delova</div>
                  </div>
                  <div>
                    <div style={{ color: '#fff', fontSize: '18px', fontWeight: 700 }}>⭐ {supplier.rating}</div>
