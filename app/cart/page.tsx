...
export default function CartPage() {
  ...
  const remainingForFree = freeShipping ? 0 : FREE_SHIPPING_THRESHOLD - subtotal;
    ...
    <div style={{ background: '#0c0d0f', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '32px 16px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
          <h1 style={{ color: '#fff', fontSize: '28px', fontWeight: 800, margin: 0 }}>Vasa korpa <span style={{ color: '#aaa', fontWeight: 400, fontSize: '18px' }}>({count})</span></h1>
          <button onClick={clear} aria-label="Obrisi sve iz korpe" style={{ background: 'none', border: '1px solid #2a2b2f', borderRadius: '8px', padding: '8px 16px', color: '#aaa', fontSize: '13px', cursor: 'pointer' }}>
            Obrisi korpu
          </button>
        </div>
        ...
        <div className="cart-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: '24px', alignItems: 'start' }}>
              ...
              return (
                <div key={item.part_id} className="cart-item-row" style={{ display: 'flex', gap: '14px', background: '#1a1b1f', borderRadius: '12px', padding: '16px', border: '1px solid #2a2b2f', alignItems: 'center' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ color: '#aaa', fontSize: '12px', marginBottom: '2px' }}>{item.brand} &middot; {item.part_number}</div>
                  ...
                  <div className="cart-item-actions" style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '8px', flexShrink: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <button onClick={() => updateQty(item.part_id, Math.max(1, item.quantity - 1))} aria-label="Smanji kolicinu" style={{ width: '28px', height: '28px', background: '#252629', border: '1px solid #2a2b2f', borderRadius: '6px', color: '#fff', fontSize: '16px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>&minus;</button>
                      <span style={{ color: '#fff', fontSize: '14px', fontWeight: 600, minWidth: '24px', textAlign: 'center' }}>{item.quantity}</span>
                      <button onClick={() => updateQty(item.part_id, item.quantity + 1)} aria-label="Povecaj kolicinu" style={{ width: '28px', height: '28px', background: '#252629', border: '1px solid #2a2b2f', borderRadius: '6px', color: '#fff', fontSize: '16px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>+</button>
                    </div>
                    <div style={{ color: '#fff', fontSize: '15px', fontWeight: 700 }}>{lineTotal.toLocaleString('sr-RS')} {item.price_currency}</div>
                    <button onClick={() => remove(item.part_id)} aria-label={`Ukloni ${item.name}`} style={{ background: 'none', border: 'none', color: '#ef4444', fontSize: '18px', cursor: 'pointer', padding: '2px 6px' }}>&times;</button>
                  </div>
                </div>
              );
            })}
          ...
          <div className="cart-summary" style={{ background: '#1a1b1f', borderRadius: '12px', padding: '20px', border: '1px solid #2a2b2f', position: 'sticky', top: '80px' }}>
            <h2 style={{ color: '#fff', fontSize: '18px', fontWeight: 700, margin: '0 0 16px' }}>Pregled porudzbine</h2>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px', gap: '12px' }}>
              ...
              <span style={{ color: freeShipping ? '#22c55e' : '#fff', fontSize: '13px', fontWeight: 600, textAlign: 'right' }}>
                {freeShipping ? 'Besplatna dostava' : `${shipping} RSD`}
              </span>
            </div>

            {!freeShipping && remainingForFree > 0 && (
              <div style={{ background: 'rgba(249,55,44,0.08)', border: '1px solid rgba(249,55,44,0.2)', borderRadius: '8px', padding: '10px 12px', marginBottom: '10px' }}>
                <p style={{ color: '#f9372c', fontSize: '12px', margin: 0 }}>
                  Dodajte jos {remainingForFree.toLocaleString('sr-RS')} {currency} za besplatnu dostavu!
                </p>
              </div>
            )}
