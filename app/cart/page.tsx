                ...
                <div key={item.part_id} className="cart-item-row" style={{ display: 'flex', gap: '14px', background: '#1a1b1f', borderRadius: '12px', padding: '16px', border: '1px solid #2a2b2f', alignItems: 'center' }}>
                  ...
                  <div className="cart-item-actions" style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '8px', flexShrink: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <button data-testid="qty-dec" aria-label="Smanji količinu" onClick={() => updateQty(item.part_id, Math.max(1, item.quantity - 1))} style={{ width: '28px', height: '28px', background: '#252629', border: '1px solid #2a2b2f', borderRadius: '6px', color: '#fff', fontSize: '16px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>−</button>
                      <span style={{ color: '#fff', fontSize: '14px', fontWeight: 600, minWidth: '24px', textAlign: 'center' }}>{item.quantity}</span>
                      <button data-testid="qty-inc" aria-label="Povećaj količinu" onClick={() => updateQty(item.part_id, item.quantity + 1)} style={{ width: '28px', height: '28px', background: '#252629', border: '1px solid #2a2b2f', borderRadius: '6px', color: '#fff', fontSize: '16px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>+</button>
                    </div>
          ...
          <div className="cart-summary" style={{ background: '#1a1b1f', borderRadius: '12px', padding: '20px', border: '1px solid #2a2b2f', position: 'sticky', top: '80px' }}>
            ...
            {shipping > 0 && (
              <div style={{ marginBottom: '10px' }}>
                ...
                <div style={{ height: '4px', background: '#2a2b2f', borderRadius: '2px', overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${Math.min(100, (subtotal / freeShippingThreshold) * 100)}%`, background: '#f9372c', borderRadius: '2px', transition: 'width 0.3s ease' }} />
                </div>
              </div>
            )}
