...
function FreeShippingBar({ subtotal }: { subtotal: number }) {
  ...
  if (subtotal >= threshold) {
    return (
      <div style={{ background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.3)', borderRadius: '10px', padding: '12px 16px', marginBottom: '16px' }}>
        <p style={{ color: '#22c55e', fontSize: '13px', fontWeight: 600, margin: 0 }}>
          ✓ Besplatna dostava ostvarena!
        </p>
      </div>
    );
  }
  ...
  return (
    <div style={{ background: '#252629', borderRadius: '10px', padding: '12px 16px', marginBottom: '16px' }}>
      <p style={{ color: '#aaa', fontSize: '12px', margin: '0 0 8px' }}>
        Još <span style={{ color: '#f9372c', fontWeight: 700 }}>{remaining.toLocaleString('sr-RS')} RSD</span> do besplatne dostave
      </p>
      <div style={{ height: '4px', background: '#1a1b1f', borderRadius: '2px', overflow: 'hidden' }}>
        <div style={{ height: '100%', width: `${progress}%`, background: 'linear-gradient(90deg, #f9372c, #ff6b3d)', borderRadius: '2px', transition: 'width 0.3s ease' }} />
      </div>
    </div>
  );
}
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '32px 16px' }}>
        ...
        <div className="cart-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: '24px', alignItems: 'start' }}>
          ...
          <div className="cart-summary" style={{ background: '#1a1b1f', borderRadius: '12px', padding: '20px', border: '1px solid #2a2b2f', position: 'sticky', top: '80px' }}>
            ...
            <div style={{ marginTop: '14px', padding: '10px', background: '#0c0d0f', borderRadius: '8px' }}>
              <p style={{ color: '#666', fontSize: '11px', margin: 0, lineHeight: 1.5 }}>
                🔒 Sigurno plaćanje preko Stripe platforme. Podržavamo Visa, Mastercard, Apple Pay.
              </p>
            </div>
          </div>
        </div>
