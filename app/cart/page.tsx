            ...
            {shipping > 0 && (
              <div style={{ marginBottom: '14px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                  <span style={{ color: '#888', fontSize: '11px' }}>Besplatna dostava</span>
                  <span style={{ color: '#888', fontSize: '11px' }}>{Math.round((subtotal / freeShippingThreshold) * 100)}%</span>
                </div>
                <div style={{ width: '100%', height: '4px', background: '#252629', borderRadius: '2px', overflow: 'hidden' }}>
                  <div style={{ width: `${Math.min(100, (subtotal / freeShippingThreshold) * 100)}%`, height: '100%', background: subtotal >= freeShippingThreshold ? '#22c55e' : '#f9372c', borderRadius: '2px', transition: 'width 0.3s ease' }} />
                </div>
                <p style={{ color: '#888', fontSize: '11px', marginTop: '4px' }}>
                  Još {(freeShippingThreshold - subtotal).toLocaleString('sr-RS')} {currency} do besplatne dostave
                </p>
              </div>
            )}
