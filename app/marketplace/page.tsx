...
function MarketplaceContent() {
  ...
  const fetchParts = useCallback(async (currentPage: number) => {
    ...
    try {
      ...
      setParts([]);
      setError('Greška pri učitavanju delova. Pokušajte ponovo.');
    } finally {
      setLoading(false);
    }
  }, [filterMake, filterCategory, filterInStock, filterMinPrice, filterMaxPrice, sortBy, searchQuery]);
  ...
  const hasActiveFilters = filterMake || filterCategory || filterInStock || filterMinPrice || filterMaxPrice || searchQuery || availOnly;
    ...
    <div style={s.page}>
      ...
      <div style={{ display: 'none', padding: '12px 16px', background: '#0c0d0f' }} className="mobile-filter-toggle">
        <button
          ...
          style={{ width: '100%', padding: '10px', background: '#1a1b1f', border: '1px solid #333', borderRadius: '8px', color: '#fff', cursor: 'pointer', fontSize: '14px', fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
        >
          <span aria-hidden="true">&#9776;</span> Filteri {hasActiveFilters ? '(aktivni)' : ''}
        </button>
      </div>
      ...
      <style>{`
        @media (max-width: 768px) {
          ...
          .marketplace-sidebar.sidebar-open { display: block !important; position: static !important; }
          .mobile-filter-toggle { display: block !important; }
        }
      `}</style>

      <div className="marketplace-layout" style={s.container}>
        <div className={`marketplace-sidebar${sidebarOpen ? ' sidebar-open' : ''}`} style={s.sidebar}>
        ...
        <div>
          ...
          {error && (
            <div style={{ background: 'rgba(239, 68, 68, 0.12)', border: '1px solid #ef4444', borderRadius: '8px', padding: '14px 18px', color: '#ef4444', fontSize: '14px', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span>{error}</span>
              <button onClick={() => fetchParts(page)} style={{ background: '#ef4444', border: 'none', borderRadius: '6px', color: '#fff', padding: '6px 14px', cursor: 'pointer', fontSize: '13px', fontWeight: 600 }}>
                Pokušaj ponovo
              </button>
            </div>
          )}
          ...
          {loading ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '16px' }}>
              {displayParts.map((part, idx) => {
                ...
                return (
                  <div key={part.id} style={{ ...s.card, border: compareList.includes(part.id) ? '2px solid #f9372c' : '2px solid transparent', animation: 'fadeIn 0.3s ease-out', animationFillMode: 'backwards', animationDelay: `${idx * 30}ms` }}>
                    ...
                    <div style={{ padding: '12px' }}>
                      ...
                      <div style={{ display: 'flex', gap: '6px', marginTop: '8px' }}>
                        ...
                        <button onClick={() => toggleCompare(part.id)} aria-label={compareList.includes(part.id) ? 'Ukloni iz poređenja' : 'Dodaj za poređenje'} style={{ padding: '8px', background: compareList.includes(part.id) ? '#f9372c' : '#333', border: 'none', borderRadius: '8px', color: '#fff', cursor: 'pointer', fontSize: '13px' }}>&#8776;</button>
                      </div>
                    </div>
                  </div>
                );
          ...
          {!loading && displayParts.length === 0 && !error && (
            <div style={{ textAlign: 'center', padding: '60px 20px' }}>
              <p style={{ fontSize: '48px' }}>&#128269;</p>
              <p style={{ fontSize: '18px', color: '#aaa', marginBottom: '16px' }}>
                {searchQuery ? `Nema rezultata za "${searchQuery}"` : 'Nema rezultata za date filtere'}
              </p>
              {hasActiveFilters && (
                <button onClick={resetFilters} style={{ marginTop: '8px', padding: '10px 24px', background: '#f9372c', border: 'none', borderRadius: '8px', color: '#fff', cursor: 'pointer', fontSize: '14px' }}>
                  Resetuj filtere
                </button>
              )}
            </div>
