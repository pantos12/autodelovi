...
function MarketplaceContent() {
  ...
  const sidebarContent = (
    <>
      ...
      <div style={{ marginBottom: '16px' }}>
        <label style={s.label}>Opseg cena (RSD)</label>
  ...
  return (
    <div style={s.page}>
      ...
      <div style={{ display: 'none', padding: '12px 16px', background: '#0c0d0f', position: 'sticky', top: '64px', zIndex: 50 }} className="mobile-filter-bar">
        <button
          onClick={() => setSidebarOpen(!sidebarOpen)}
          style={{ width: '100%', padding: '10px', background: '#1a1b1f', border: '1px solid #333', borderRadius: '8px', color: '#fff', cursor: 'pointer', fontSize: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
        >
          <span>Filteri</span>
          {activeFilterCount > 0 && (
            <span style={{ background: '#f9372c', color: '#fff', fontSize: '11px', fontWeight: 700, borderRadius: '10px', padding: '1px 7px' }}>{activeFilterCount}</span>
          )}
        </button>
      </div>
      ...
      {sidebarOpen && (
        <div className="mobile-sidebar-drawer" style={{ display: 'none', position: 'fixed', inset: 0, zIndex: 200 }}>
          <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.6)' }} onClick={() => setSidebarOpen(false)} />
          <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: '300px', maxWidth: '85vw', background: '#1a1b1f', padding: '20px', overflowY: 'auto', zIndex: 201 }}>
