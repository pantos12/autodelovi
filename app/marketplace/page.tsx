...
function MarketplaceContent() {
  ...
  const activeFilters = [filterMake, filterCategory, filterInStock, availOnly, minPrice, maxPrice, searchQuery].filter(Boolean).length;
  ...
  return (
    <div style={s.page}>
      <style>{`
        ...
        .mp-sidebar-overlay { position: fixed; inset: 0; z-index: 200; background: rgba(0,0,0,0.6); backdrop-filter: blur(4px); }
        .mp-sidebar-drawer { position: fixed; top: 0; left: 0; bottom: 0; width: 300px; max-width: 85vw; z-index: 201; background: #1a1b1f; padding: 20px; overflow-y: auto; transform: translateX(-100%); transition: transform 0.25s ease; }
        .mp-sidebar-drawer.open { transform: translateX(0); }
      `}</style>

      {/* Mobile filter drawer */}
      {sidebarOpen && <div className="mp-sidebar-overlay" onClick={() => setSidebarOpen(false)} />}
      <div className={`mp-sidebar-drawer ${sidebarOpen ? 'open' : ''}`}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <h3 style={{ color: '#fff', fontSize: '18px', fontWeight: 700, margin: 0 }}>Filteri</h3>
          <button onClick={() => setSidebarOpen(false)} style={{ background: 'none', border: 'none', color: '#aaa', fontSize: '24px', cursor: 'pointer', padding: '4px' }}>✕</button>
        </div>
        {sidebarContent}
      </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
              ...
              <button
                ...
                style={{ display: 'none', alignItems: 'center', gap: '6px', padding: '8px 14px', background: '#1a1b1f', border: '1px solid #333', borderRadius: '8px', color: '#fff', cursor: 'pointer', fontSize: '13px', fontWeight: 600 }}
              >
