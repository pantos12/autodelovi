    ...
    <div style={s.page}>
      <style>{`
        @media (max-width: 768px) {
          ...
          .marketplace-mobile-toggle { display: flex !important; }
        }
        @media (min-width: 769px) {
          .marketplace-mobile-toggle { display: none !important; }
          .marketplace-mobile-sidebar { display: none !important; }
        }
      `}</style>
      <div className="marketplace-container" style={s.container}>
        {/* Mobile filter toggle */}
        <button
          className="marketplace-mobile-toggle"
          onClick={() => setSidebarOpen(!sidebarOpen)}
          style={{ display: 'none', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '10px', background: '#1a1b1f', border: '1px solid #333', borderRadius: '8px', color: '#fff', cursor: 'pointer', fontSize: '14px', gridColumn: '1 / -1' }}
        >
          🔧 Filteri {activeFilterCount > 0 && <span style={{ background: '#f9372c', color: '#fff', borderRadius: '10px', padding: '2px 8px', fontSize: '11px', fontWeight: 700 }}>{activeFilterCount}</span>}
        </button>

        {/* Mobile sidebar */}
        {sidebarOpen && (
          <div className="marketplace-mobile-sidebar" style={{ ...s.sidebar, position: 'static', gridColumn: '1 / -1' } as React.CSSProperties}>
            {sidebarContent}
          </div>
        )}
          ...
          {loading ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '16px' }}>
              {displayParts.map((part, idx) => {
                ...
                return (
                  <div key={part.id} className="part-card" style={{ ...s.card, border: compareList.includes(part.id) ? '2px solid #ff4d00' : '2px solid transparent' }}>
