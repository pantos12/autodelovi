...
import { bandEmoji, bandLabel, computeBand, type Band } from '@/lib/confidence';
...
function bandForPart(part: PartWithBestOffer): Band {
  if (part.best_offer) {
    return part.best_offer.band;
  }
  if ((part.stock_quantity ?? 0) > 0) return 'verified';
  return 'inquiry';
}

function lastCheckedText(part: PartWithBestOffer): string {
  const timestamp = part.best_offer?.last_seen_at ?? part.scraped_at;
  if (!timestamp) return 'Nema podataka';
  const ageMs = Date.now() - new Date(timestamp).getTime();
  const mins = Math.floor(ageMs / 60_000);
  if (mins < 1) return 'Upravo provereno';
...
function MarketplaceContent() {
  ...
  const [parts, setParts] = useState<PartWithBestOffer[]>([]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const endpoint = debouncedSearch && debouncedSearch.length >= 2 ? '/api/search' : '/api/parts';
  ...
  }, [filterMake, filterCategory, filterInStock, sortBy, debouncedSearch, debouncedMinPrice, debouncedMaxPrice, page]);

  useEffect(() => {
    setPage(1);
  }, [filterMake, filterCategory, filterInStock, sortBy, debouncedSearch, debouncedMinPrice, debouncedMaxPrice]);
  ...
  const hasActiveFilters = filterMake || filterCategory || filterInStock || availOnly || minPrice || maxPrice || searchInput;
  ...
  const sidebarContent = (
    <>
      <div style={{ marginBottom: '20px' }}>
        ...
        />
        {searchInput && (
          <button type="button" onClick={() => setSearchInput('')} style={{ marginTop: '8px', background: 'none', border: 'none', color: '#f9372c', fontSize: '12px', cursor: 'pointer', padding: 0 }}>
            ✕ Obriši: &quot;{searchInput}&quot;
          </button>
        )}
      </div>
      ...
      <button onClick={clearFilters} style={{ width: '100%', padding: '8px', background: '#333', border: 'none', borderRadius: '8px', color: '#fff', cursor: 'pointer', fontSize: '13px' }}>
        Resetuj sve
      </button>
    </>
  );
    ...
    <div style={s.page}>
      ...
      <div style={{ display: 'none', padding: '12px 16px', maxWidth: '1200px', margin: '0 auto' }} className="mobile-filter-toggle">
        <button
          ...
          style={{ width: '100%', padding: '10px 16px', background: '#1a1b1f', border: '1px solid #333', borderRadius: '8px', color: '#fff', fontSize: '14px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
        >
          <span>Filteri</span>
          {hasActiveFilters && <span style={{ background: '#f9372c', borderRadius: '50%', width: '8px', height: '8px', display: 'inline-block' }} />}
        </button>
      </div>
      ...
      <style>{`
        @media (max-width: 768px) {
          ...
          .marketplace-sidebar-wrapper { display: ${sidebarOpen ? 'block' : 'none'} !important; }
        }
      `}</style>

      <div style={s.container} className="marketplace-grid-layout">
        <div style={s.sidebar} className="marketplace-sidebar-wrapper marketplace-sidebar">
          {sidebarContent}
        </div>
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
            <p style={{ color: '#aaa', fontSize: '14px' }}>
              {loading ? 'Učitavanje...' : debouncedSearch ? `${total} rezultata za "${debouncedSearch}"` : `${total} delova`}
            </p>
                ...
                return (
                  <div key={part.id} data-testid="part-card" style={{ ...s.card, border: compareList.includes(part.id) ? '2px solid #ff4d00' : '2px solid transparent', transition: 'border-color 0.15s' }}>
                    ...
                    <div style={{ padding: '12px' }}>
                      ...
                      <h3 style={{ color: '#fff', fontSize: '14px', marginBottom: '8px', lineHeight: '1.3', overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' } as React.CSSProperties}>{part.name_sr || part.name}</h3>
                      ...
                      <div style={{ marginBottom: '6px' }}>
                        {band === 'inquiry' ? (
                          ...
                          <>
                            <AddToCartButton part={part} full inStock={true} />
                            {band === 'likely' && (
          ...
          {!loading && displayParts.length === 0 && (
            <div style={{ textAlign: 'center', padding: '60px 20px' }}>
              ...
              <p style={{ fontSize: '18px', color: '#aaa', marginBottom: '8px' }}>
                {debouncedSearch ? `Nema rezultata za "${debouncedSearch}"` : 'Nema rezultata za date filtere'}
              </p>
              {hasActiveFilters && (
                <button onClick={clearFilters} style={{ marginTop: '16px', padding: '10px 24px', background: '#f9372c', border: 'none', borderRadius: '8px', color: '#fff', cursor: 'pointer', fontSize: '14px' }}>
                  Resetuj filtere
                </button>
              )}
            </div>
