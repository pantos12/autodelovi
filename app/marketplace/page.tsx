...
function MarketplaceContent() {
  ...
  return (
    <div style={s.page}>
      ...
      <div className="mp-mobile-filter-btn" style={{ display: 'none', position: 'sticky', top: '64px', zIndex: 50, background: '#0c0d0f', padding: '12px 16px', borderBottom: '1px solid #252629', alignItems: 'center', justifyContent: 'space-between' }}>
        <button onClick={() => setFiltersOpen(!filtersOpen)} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 16px', background: '#1a1b1f', border: '1px solid #333', borderRadius: '8px', color: '#fff', cursor: 'pointer', fontSize: '13px', fontWeight: 600 }}>
          <span>Filteri</span>
          {activeFilterCount > 0 && (
            <span style={{ background: '#f9372c', color: '#fff', fontSize: '10px', fontWeight: 700, borderRadius: '50%', width: '18px', height: '18px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{activeFilterCount}</span>
          )}
        </button>
      ...
      {filtersOpen && (
        <div className="mp-mobile-drawer" style={{ position: 'fixed', inset: 0, top: '64px', background: 'rgba(0,0,0,0.6)', zIndex: 90 }} onClick={() => setFiltersOpen(false)}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#1a1b1f', width: '85%', maxWidth: '340px', height: '100%', padding: '20px', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <h3 style={{ color: '#fff', fontSize: '16px', margin: 0 }}>Filteri</h3>
              <button onClick={() => setFiltersOpen(false)} style={{ background: 'none', border: 'none', color: '#aaa', fontSize: '22px', cursor: 'pointer', padding: '4px' }}>✕</button>
            </div>
            {sidebarContent}
          </div>
        </div>
