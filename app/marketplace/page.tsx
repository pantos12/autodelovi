...
function MarketplaceContent() {
  ...
  const prevFilterRef = useRef({ filterMake, filterCategory, filterInStock, sortBy, searchQuery });

  useEffect(() => {
    const prev = prevFilterRef.current;
    const filtersChanged = prev.filterMake !== filterMake ||
      prev.filterCategory !== filterCategory ||
      prev.filterInStock !== filterInStock ||
      prev.sortBy !== sortBy ||
      prev.searchQuery !== searchQuery;
    prevFilterRef.current = { filterMake, filterCategory, filterInStock, sortBy, searchQuery };
                  ...
                  <div key={part.id} style={{ ...s.card, border: compareList.includes(part.id) ? '2px solid #ff4d00' : '2px solid transparent' }}>
                    ...
                    <div style={{ padding: '12px' }}>
                      ...
                      <div style={{ display: 'flex', gap: '6px', marginTop: '8px' }}>
                        ...
                        <button aria-label={compareList.includes(part.id) ? 'Ukloni iz poredjenja' : 'Dodaj u poredjenje'} onClick={() => toggleCompare(part.id)} style={{ padding: '8px', background: compareList.includes(part.id) ? '#ff4d00' : '#333', border: 'none', borderRadius: '8px', color: '#fff', cursor: 'pointer', fontSize: '13px' }}>≈</button>
                      </div>
