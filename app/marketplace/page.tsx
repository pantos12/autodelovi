    ...
    <div style={s.page}>
      ...
      <div className="mp-mobile-filter-btn" style={{ display: 'none', position: 'sticky', top: '64px', zIndex: 50, background: '#0c0d0f', padding: '12px 16px', borderBottom: '1px solid #252629', alignItems: 'center', justifyContent: 'space-between' }}>
        <button
          ...
          style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 16px', background: '#1a1b1f', border: '1px solid #333', borderRadius: '8px', color: '#fff', cursor: 'pointer', fontSize: '14px' }}
        >
          <span>☰ Filteri</span>
          {activeFilterCount > 0 && (
        ...
        <select style={{ ...s.select, width: 'auto', fontSize: '13px' }} value={sortBy} onChange={e => setSortBy(e.target.value)}>
      ...
      {filtersOpen && (
        <div className="mp-mobile-overlay" style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(0,0,0,0.6)' }} onClick={() => setFiltersOpen(false)}>
          <div
            style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: '300px', maxWidth: '85vw', background: '#1a1b1f', padding: '20px', overflowY: 'auto' }}
                  ...
                  <div key={part.id} style={{ ...s.card, border: compareList.includes(part.id) ? '2px solid #ff4d00' : '2px solid transparent' }}>
                    ...
                    <div style={{ padding: '12px' }}>
                      ...
                      <div style={{ display: 'flex', gap: '6px', marginTop: '8px' }}>
                        ...
                        <button onClick={() => toggleCompare(part.id)} style={{ padding: '8px', background: compareList.includes(part.id) ? '#ff4d00' : '#333', border: 'none', borderRadius: '8px', color: '#fff', cursor: 'pointer', fontSize: '13px' }} aria-label="Uporedi">≈</button>
                      </div>
                    </div>
                  </div>
                );
