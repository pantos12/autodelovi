...
function MarketplaceContent() {
  ...
  const hasActiveFilters = filterMake || filterCategory || filterInStock || availOnly || minPrice || maxPrice || searchQuery;
    ...
    <>
      <form onSubmit={handleSearch} style={{ marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '6px' }}>
          ...
          />
          <button type="submit" data-testid="search-submit" style={{ padding: '8px 12px', background: '#f9372c', border: 'none', borderRadius: '8px', color: '#fff', cursor: 'pointer', fontSize: '14px', flexShrink: 0 }}>
            🔍
          </button>
        </div>
        {searchQuery && (
      ...
      <div style={{ marginBottom: '16px' }}>
        ...
        <select style={s.select} value={filterMake} onChange={e => setFilterMake(e.target.value)} data-testid="filter-make">
          <option value="">Sve marke</option>
          {['Volkswagen','BMW','Mercedes','Audi','Opel','Renault','Peugeot','Fiat','Toyota','Ford','Skoda','Seat','Hyundai','Kia','Citroen','Mazda','Nissan','Honda'].map(m => (
      ...
      <div style={{ marginBottom: '16px' }}>
        ...
        <select style={s.select} value={filterCategory} onChange={e => setFilterCategory(e.target.value)} data-testid="filter-category">
      ...
      <div style={{ marginBottom: '16px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <input
            ...
            min={0}
            style={{ ...s.select, flex: 1, padding: '8px 10px' }}
          />
          <input
            type="number"
      ...
      <button onClick={clearAllFilters} data-testid="filter-reset" style={{ width: '100%', padding: '8px', background: hasActiveFilters ? '#f9372c' : '#333', border: 'none', borderRadius: '8px', color: '#fff', cursor: 'pointer', fontSize: '13px', transition: 'background 0.15s' }}>
        Resetuj sve {hasActiveFilters ? '✕' : ''}
      </button>
    </>
  );
  ...
  return (
    <div style={s.page}>
      ...
      <div style={{ display: 'none', padding: '12px 16px', background: '#1a1b1f', borderBottom: '1px solid #252629', position: 'sticky', top: '64px', zIndex: 50 }} className="mobile-filter-toggle">
        <button
          onClick={() => setMobileFiltersOpen(!mobileFiltersOpen)}
          data-testid="mobile-filter-toggle"
          style={{ width: '100%', padding: '10px', background: '#252629', border: '1px solid #333', borderRadius: '8px', color: '#fff', cursor: 'pointer', fontSize: '14px', fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
        >
          🔧 Filteri {hasActiveFilters ? `(${[filterMake, filterCategory, searchQuery, filterInStock && 'stanje', availOnly && 'dostupno', minPrice && 'min', maxPrice && 'max'].filter(Boolean).length})` : ''}
        </button>
      </div>
      ...
      <div style={s.container} className="marketplace-layout">
        ...
        <div style={{ ...s.sidebar, display: mobileFiltersOpen ? 'block' : undefined }} className="marketplace-sidebar" data-testid="marketplace-sidebar">
          {filterContent}
        </div>

        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
            <p style={{ color: '#aaa', fontSize: '14px' }} data-testid="results-count">
              {loading ? 'Učitavanje...' : searchQuery ? `${total} rezultata za "${searchQuery}"` : `${total} delova`}
            </p>
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
              <select style={{ ...s.select, width: 'auto' }} value={sortBy} onChange={e => setSortBy(e.target.value)} data-testid="sort-select">
                <option value="price_asc">Cena: niža → viša</option>
                <option value="price_desc">Cena: viša → niža</option>
                <option value="newest">Najnovije</option>
              </select>
              {compareList.length > 0 && (
                <button onClick={() => router.push('/comparison?ids=' + compareList.join(','))} data-testid="compare-go" style={{ padding: '8px 16px', background: '#ff4d00', border: 'none', borderRadius: '8px', color: '#fff', cursor: 'pointer', fontSize: '13px' }}>
          ...
          {loading ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '16px' }}>
              {displayParts.map((part, idx) => {
                ...
                return (
                  <div key={part.id} data-testid="part-card" style={{ ...s.card, border: compareList.includes(part.id) ? '2px solid #ff4d00' : '2px solid transparent', transition: 'border-color 0.15s, transform 0.15s', cursor: 'default' }}>
                    <div style={{ position: 'relative', background: '#252629', height: '140px', overflow: 'hidden' }}>
                      <Link href={partUrl} style={{ display: 'block', position: 'relative', width: '100%', height: '100%' }}>
                    ...
                    <div style={{ padding: '12px' }}>
                      ...
                      <p data-testid="stock-status" style={{ color: inStock ? '#22c55e' : '#ef4444', fontSize: '12px', marginBottom: '10px' }}>{inStock ? 'Na stanju' : 'Nema na stanju'}</p>
                      ...
                      <div style={{ display: 'flex', gap: '6px', marginTop: '8px' }}>
                        ...
                        <button onClick={() => toggleCompare(part.id)} data-testid="compare-toggle" title="Uporedi" style={{ padding: '8px', background: compareList.includes(part.id) ? '#ff4d00' : '#333', border: 'none', borderRadius: '8px', color: '#fff', cursor: 'pointer', fontSize: '13px' }}>≈</button>
                      </div>

                      <p style={{ color: '#666', fontSize: '10px', marginTop: '8px' }}>
                        {timeAgo(part.scraped_at || part.updated_at)}
                      </p>
                    </div>
                  </div>
                );
          ...
          {!loading && displayParts.length === 0 && (
            <div style={{ textAlign: 'center', padding: '60px 20px' }}>
              ...
              {hasActiveFilters && (
                <button onClick={clearAllFilters} style={{ marginTop: '16px', padding: '10px 24px', background: '#f9372c', border: 'none', borderRadius: '8px', color: '#fff', cursor: 'pointer', fontSize: '14px' }}>
                  Resetuj filtere
                </button>
              )}
            </div>
          ...
          {!loading && totalPages > 1 && (
            <div data-testid="pagination" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '6px', marginTop: '32px', flexWrap: 'wrap' }}>
              <button
          ...
          {!loading && total > PER_PAGE && (
            <div style={{ textAlign: 'center', marginTop: '16px' }}>
              <button
                ...
                style={{ background: 'none', border: 'none', color: '#666', fontSize: '12px', cursor: 'pointer', padding: '8px' }}
              >
                ↑ Nazad na vrh
              </button>
            </div>
