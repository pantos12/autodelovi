...
function MarketplaceContent() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        if (searchQuery && searchQuery.length >= 2) {
          ...
          if (priceApplied.min) params.set('min_price', priceApplied.min);
          if (priceApplied.max) params.set('max_price', priceApplied.max);
    ...
    load();
  }, [filterMake, filterCategory, filterInStock, sortBy, searchQuery, page, priceApplied]);

  useEffect(() => {
    setPage(1);
  }, [filterMake, filterCategory, filterInStock, sortBy, searchQuery, priceApplied]);
  ...
  const activeFilterCount = [
    filterMake, filterCategory, filterInStock, availOnly, priceApplied.min, priceApplied.max, searchQuery,
    ...
    <>
      ...
      <div style={{ marginBottom: '16px' }}>
        ...
        <button
          ...
          style={{ marginTop: '6px', width: '100%', padding: '6px', background: '#252629', border: '1px solid #333', borderRadius: '6px', color: '#ccc', cursor: 'pointer', fontSize: '12px' }}
        >
          Primeni cenu
        </button>
      </div>
      ...
      <button onClick={clearAllFilters} style={{ width: '100%', padding: '8px', background: '#333', border: 'none', borderRadius: '8px', color: '#fff', cursor: 'pointer', fontSize: '13px' }}>
        Resetuj sve {activeFilterCount > 0 && `(${activeFilterCount})`}
      </button>
    </>
  );
  ...
  return (
    <div style={s.page}>
      ...
      <div className="mp-grid" style={{ maxWidth: '1200px', margin: '0 auto', padding: '24px 16px', display: 'grid', gridTemplateColumns: '240px 1fr', gap: '24px' }}>
        {/* Desktop sidebar */}
        <div className="mp-sidebar" style={{ background: '#1a1b1f', borderRadius: '12px', padding: '20px', height: 'fit-content', position: 'sticky', top: '80px' }}>
          {sidebarContent}
        </div>
        ...
        <div>
          ...
          <button
            ...
            style={{ display: 'none', alignItems: 'center', justifyContent: 'center', gap: '8px', width: '100%', padding: '12px', background: '#1a1b1f', border: '1px solid #333', borderRadius: '10px', color: '#fff', cursor: 'pointer', fontSize: '14px', fontWeight: 600, marginBottom: '16px' }}
          >
            <span>Filteri</span>
            {activeFilterCount > 0 && (
            ...
            <span style={{ marginLeft: 'auto', fontSize: '18px', transform: mobileFiltersOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>▼</span>
          </button>

          {/* Mobile filter drawer */}
          {mobileFiltersOpen && (
            <div className="mp-mobile-drawer" style={{ background: '#1a1b1f', borderRadius: '12px', padding: '20px', marginBottom: '16px', border: '1px solid #333' }}>
              {sidebarContent}
            </div>
          )}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
            <p style={{ color: '#aaa', fontSize: '14px' }}>
              {loading ? 'Ucitavanje...' : searchQuery ? `${total} rezultata za "${searchQuery}"` : `${total} delova`}
            </p>
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
              <select style={{ ...s.select, width: 'auto' }} value={sortBy} onChange={e => setSortBy(e.target.value)}>
                <option value="price_asc">Cena: niza → visa</option>
                <option value="price_desc">Cena: visa → niza</option>
          ...
          {loading ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '16px' }}>
              {displayParts.map((part, idx) => {
                ...
                return (
                  <div key={part.id} style={{ ...s.card, border: compareList.includes(part.id) ? '2px solid #ff4d00' : '2px solid transparent', transition: 'transform 0.15s, box-shadow 0.15s' }}
                    onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = 'translateY(-2px)'; (e.currentTarget as HTMLElement).style.boxShadow = '0 4px 12px rgba(0,0,0,0.3)'; }}
                    onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = 'translateY(0)'; (e.currentTarget as HTMLElement).style.boxShadow = 'none'; }}>
                    <Link href={partUrl} style={{ textDecoration: 'none', color: 'inherit' }}>
                    ...
                    <div style={{ padding: '12px' }}>
                      ...
                      <p style={{ color: '#666', fontSize: '10px', marginTop: '8px' }}>
                        {formatTimeSince(part.updated_at || part.scraped_at)}
                      </p>
                    </div>
                  </div>
                );
...
export default function Marketplace() {
  return (
    <Suspense fallback={<div style={{ padding: '60px', textAlign: 'center', color: '#aaa' }}>Ucitavanje...</div>}>
      <MarketplaceContent />
    </Suspense>
  );
}
