...
function bandForPart(part: Part): Band {
  ...
  if (!updatedAt) return qty > 0 ? 'verified' : 'inquiry';
    ...
    <div style={s.page}>
      ...
      <div style={s.container}>
        ...
        <button
          ...
          style={{ display: 'block', width: '100%', padding: '12px', background: '#1a1b1f', border: '1px solid #333', borderRadius: '8px', color: '#fff', cursor: 'pointer', fontSize: '14px', marginBottom: '16px', textAlign: 'center' }}
        >
          {mobileFilters ? '✕ Zatvori filtere' : '☰ Filteri i pretraga'}
        </button>

        {/* Mobile filter panel */}
        {mobileFilters && (
          <div className="mp-sidebar-mobile" style={{ ...s.sidebar, position: 'static', marginBottom: '16px' }}>
            {sidebarContent}
          </div>
        )}
                    ...
                    <div key={part.id} data-testid="part-card" style={{ ...s.card, border: compareList.includes(part.id) ? '2px solid #ff4d00' : '2px solid transparent' }}>
                      ...
                      <div style={{ padding: '12px' }}>
                        ...
                        <p style={{ color: '#666', fontSize: '10px', marginTop: '8px' }}>
                          Provereno: {timeAgo((part as any).updated_at)}
                        </p>
                      </div>
                    </div>
                  );
