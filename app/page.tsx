...
export const metadata: Metadata = {
  ...
  description: 'Agregiramo delimicno skladiste od 50,000+ delova od 200+ proverenih dobavljaca sirom Srbije. VW, BMW, Mercedes, Audi, Opel i jos mnogo.',
  ...
  return (
    <>
      ...
      <div style={{ background: '#0c0d0f', minHeight: '100vh', color: '#fff', fontFamily: "'Inter','Helvetica Neue',sans-serif", position: 'relative', overflow: 'hidden' }}>
          ...
          <div className="categories-grid" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: '12px' }}>
            {categories.map(cat => (
              <Link href={'/categories/' + cat.slug} key={cat.slug} style={{ textDecoration: 'none' }}>
                <div className="cat-card" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: cat.large ? '28px' : '24px', height: '100%', transition: 'border-color 0.2s' }}>
        ...
        {/* TRUST INDICATORS */}
        <section className="section-pad" style={{ position: 'relative', zIndex: 5, maxWidth: '900px', margin: '0 auto', padding: '0 24px 80px' }}>
          <div className="trust-stats" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', textAlign: 'center' }}>
            {[
              ...
              <div key={i} style={{ padding: '20px 12px' }}>
                <div style={{ color: '#f9372c', fontSize: '28px', fontWeight: 800, marginBottom: '4px' }}>{stat.value}</div>
                <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: '12px', fontWeight: 500 }}>{stat.label}</div>
              </div>
            ))}
          </div>
        </section>
