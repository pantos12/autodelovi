...
export async function getParts(params: {
  ...
  if (q) {
    const escaped = q.replace(/[%_\\]/g, c => '\\' + c).slice(0, 200);
    query = query.or(`name.ilike.%${escaped}%,name_sr.ilike.%${escaped}%,part_number.ilike.%${escaped}%,oem_number.ilike.%${escaped}%,brand.ilike.%${escaped}%`);
  }
